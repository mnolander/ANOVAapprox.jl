...
function get_AttributeRanking( approx::fun_approx, lambda::Float64 )::Vector{Float64}
    ...
    r = zeros(d)
    nf = 0.0
    lengths = [ length(approx.U[i]) for i = 2:length(approx.U) ]
    ds = maximum(lengths)
    ...
    for i = 1:d 
        for j = 1:ds 
            for k = 1:length(approx.U)
                if (i in approx.U[k]) && (length(approx.U[k]) == j)
                    factors[i,j] += 1
                end
            end
        end
    ...
    for i = 1:length(approx.U)
        ...
        for s in u
            r[s] += 1/factors[s,length(u)] * gsi[i]
        end
    end
